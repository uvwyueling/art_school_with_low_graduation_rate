...
def mask_student_id(df, id_col='学号'):
...
# 修改列名
dim_student = stage_dim_student.rename(columns={'学号': 'uid','大一班级': 'year_1_class_num','班级': 'grad_class_num','年级': 'enroll_year' })
...
temp_fact_major_reassign = df_stg_reassign_ready[['学号','拟分流专业第一志愿','拟分流专业第二志愿','最终专业','is_reassign']]

fact_major_reassign = temp_fact_major_reassign.rename(columns={'学号': 'uid','拟分流专业第一志愿': 'first_choice','拟分流专业第二志愿': 'second_choice','最终专业': 'final_assigned_major' })
...
fact_gpa_records_stage = pd.concat([y1_gpa_ready,y234_gpa_ready], ignore_index=True)

# 调整列名
fact_gpa_records = fact_gpa_records_stage.rename(columns={'平均学分绩点': 'gpa_score','学号':'uid'})
